...
class FirebaseService {
    ...
    fun getAllDataFromFB(movies: MutableLiveData<ArrayList<Movie>>) {
        ...
        db.collection("reviews").orderBy("id", Query.Direction.ASCENDING)
            .addSnapshotListener { value, error ->
                //todo change if blocks to when block
                if (error != null) {
    ...
    fun getDataFromFb(id: Int, movie: MutableLiveData<Movie>) {
        db.collection("reviews").whereEqualTo("id", id).orderBy("id", Query.Direction.ASCENDING)
            .addSnapshotListener { value, error ->
                if (error != null) {
                    println(error.localizedMessage)
                } else {
